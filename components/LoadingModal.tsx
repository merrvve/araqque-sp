      ...
      <Modal show={openModal} onClose={() => setOpenModal(false)}>
        ...
        <Modal.Body>
          <div className="space-y-6">
            <div className="text-base leading-relaxed text-gray-500 dark:text-gray-400">
              {loadingState.fileUploaded && !loadingState.fileError ? (
                "Dosya Yüklendi"
              ) : loadingState.fileError || loadingState.questionsError ? (
                ""
              ) : (
                <>
            ...
            <div className="text-base leading-relaxed text-gray-500 dark:text-gray-400">
              {loadingState.questionsPrepared ? (
                "Test Oluşturuldu"
              ) : loadingState.questionsError || loadingState.fileError ? (
                ""
              ) : (
                <>
              ...
              {loadingState.questionsError && (
                <span>Sorular oluşturulamadı</span>
              )}
            </div>
            <p className="text-base leading-relaxed text-gray-500 dark:text-gray-400">
              {loadingState.questionsPrepared && (
                <>
                  Toplamda <strong>{loadingState.questionCount}</strong> soru
                  için <strong>{loadingState.time}</strong> dakika sürelik
                  sınavınız oluşturuldu.
                </>
              )}
            </p>
          </div>
        ...
        <Modal.Footer>
          <Button
            ...
            disabled={loadingState.questionsError}
          >
            Testi Başlat
          </Button>
          <Button
